...
const createMapOfMathMLElements = async (log, inputPath, cssPath, outputPath) => {
  ...
  const mathEntries = await page.evaluate(/* istanbul ignore next */() => {
    ...
    for (const mathNode of mathNodes) {
      ...
      // clear the inside of the element because it will be replaced once typesetting completes
      // (maybe saves memory?)
      // mathNode.innerHTML = ''
  ...
  await page.evaluate(/* istanbul ignore next */(convertedMathMLEntries) => {
